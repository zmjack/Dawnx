using System;
using System.Security.Cryptography;
using Xunit;
using System.Text;
using Dawnx.Algorithms.Generator;

namespace Dawnx.Security.Test
{
    public class AesTest1
    {
        [Fact]
        public void Test1()
        {
            var key16 = Convert.ToBase64String(Encoding.Default.GetBytes(
                RandomString.ReadOne("$w".Times(16))));
            var key24 = Convert.ToBase64String(Encoding.Default.GetBytes(
                RandomString.ReadOne("$w".Times(24))));
            var key32 = Convert.ToBase64String(Encoding.Default.GetBytes(
                RandomString.ReadOne("$w".Times(32))));
            var key48 = Convert.ToBase64String(Encoding.Default.GetBytes(
                RandomString.ReadOne("$w".Times(48))));

            var text = Guid.NewGuid().ToString();
            var data = Encoding.UTF8.GetBytes(text);

            // 128 bits (16 bytes) key
            Aes.Create().Self(_ =>
            {
                _.FromBase64String(key16);
                Assert.Equal(_.Decrypt(_.Encrypt(data)), data);
            });

            // 192 bits (24 bytes) key
            Aes.Create().Self(_ =>
            {
                _.FromBase64String(key24);
                Assert.Equal(_.Decrypt(_.Encrypt(data)), data);
            });

            // 256 bits (32 bytes) key
            Aes.Create().Self(_ =>
            {
                _.FromBase64String(key32);
                Assert.Equal(_.Decrypt(_.Encrypt(data)), data);
            });

            // Not a key
            Aes.Create().Self(_ =>
            {
                Assert.Throws<CryptographicException>(() => _.FromBase64String(key48));
            });
        }

        [Fact]
        public void Test2()
        {
            Aes.Create().Self(_ =>
            {
                _.SetEmptyIV();
                _.Key = "1234567890123456".GetBytes(Encoding.ASCII);

                Assert.Equal("0c66182ec710840065ebaa47c5e6ce90",
                    _.Encrypt<AesEmptyIVCombiner>("123".GetBytes(Encoding.UTF8)).GetHexString());
            });

            Aes.Create().Self(_ =>
            {
                var now = DateTime.Now.ToString();
                var encrypted = _.Encrypt(now.ToString().GetBytes(Encoding.UTF8)).GetBase64String();
                Assert.Equal(now, _.Decrypt(encrypted.GetBytesFromBase64()).GetString(Encoding.UTF8));
            });
        }

        [Fact]
        public void Test3()
        {
            Aes.Create().Self(_ =>
            {
                _.SetEmptyIV();
                _.Key = "075ee2b75db7c4452c21fa9b33e89101".GetBytes(Encoding.ASCII);
                _.Mode = CipherMode.ECB;

                var bytes = ""
                .GetBytesFromHexString();

                var aa = _.Decrypt<AesDefaultCombiner>(bytes).GetString(Encoding.UTF8);
            });
        }

    }
}
