...
namespace Dawnx.Net.Web
{
    public partial class HttpAccess
    {
        ...
        public TRet DeleteFor<TRet>(string url, Dictionary<string, object> updata = null)
            => JsonConvert.DeserializeObject<TRet>(Delete(url, updata));
        public TRet DeleteFor<TRet>(string url, object updata)
            => DeleteFor<TRet>(url, ObjectUtility.CovertToDictionary(updata));

        public JToken DeleteFor(string url, Dictionary<string, object> updata = null)
            => JsonConvert.DeserializeObject<JToken>(Delete(url, updata));
        public JToken DeleteFor(string url, object updata)
            => DeleteFor(url, ObjectUtility.CovertToDictionary(updata));
