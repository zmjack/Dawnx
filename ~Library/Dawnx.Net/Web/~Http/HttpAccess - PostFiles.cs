...
namespace Dawnx.Net.Web
{
    public partial class HttpAccess
    {
        ...
        public TRet PostFilesFor<TRet>(string url, Dictionary<string, object> updata = null, Dictionary<string, object> upfiles = null)
            => JsonConvert.DeserializeObject<TRet>(PostFiles(url, updata, upfiles));
        public TRet PostFilesFor<TRet>(string url, object updata, Dictionary<string, object> upfiles = null)
            => PostFilesFor<TRet>(url, ObjectUtility.CovertToDictionary(updata), upfiles);

        public JToken PostFilesFor(string url, Dictionary<string, object> updata = null, Dictionary<string, object> upfiles = null)
            => JsonConvert.DeserializeObject<JToken>(PostFiles(url, updata, upfiles));
        public JToken PostFilesFor(string url, object updata, Dictionary<string, object> upfiles = null)
            => PostFilesFor(url, ObjectUtility.CovertToDictionary(updata), upfiles);
