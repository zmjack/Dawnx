...
namespace Dawnx.Net.Web
{
    public partial class HttpAccess
    {
        ...
        public TRet PutJsonFor<TRet>(string url, Dictionary<string, object> updata = null)
            => JsonConvert.DeserializeObject<TRet>(PutJson(url, updata));
        public TRet PutJsonFor<TRet>(string url, object updata)
            => PutJsonFor<TRet>(url, ObjectUtility.CovertToDictionary(updata));

        public JToken PutJsonFor(string url, Dictionary<string, object> updata = null)
            => JsonConvert.DeserializeObject<JToken>(PutJson(url, updata));
