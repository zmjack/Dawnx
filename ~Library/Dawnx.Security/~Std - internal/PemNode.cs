...
namespace Dawnx.Security
{
    internal class PemNode
    {
        ...
        public static readonly byte[] RSA_OID =
        ...
        public static int GetLength(byte[] bytes)
        {
            switch (bytes[0])
            {
                case byte sign when sign < 0x80:
                    return sign;

                case byte sign when sign == 0x81:
                    return BitConverter.ToInt32(new[] { bytes[1], (byte)0, (byte)0, (byte)0 }, 0);

                case byte sign when sign == 0x82:
                    return BitConverter.ToInt32(new[] { bytes[2], bytes[1], (byte)0, (byte)0 }, 0);

                case byte sign when sign == 0x83:
                    return BitConverter.ToInt32(new[] { bytes[3], bytes[2], bytes[1], (byte)0 }, 0);

                case byte sign when sign == 0x84:
                    if (bytes[1] <= 0x7F)
                        return BitConverter.ToInt32(new[] { bytes[4], bytes[3], bytes[2], bytes[1] }, 0);
                    else goto default;

                default:
                    throw new ArgumentException("The length cannot greater than 0x7FFFFFFF.");
            }
        }
        ...
        public static byte[] GetLengthData(int length)
        {
            switch (length)
            {
                case int len when len < 0x80:
                    return new byte[] { Convert.ToByte(len) };

                case int len when len <= 0xFF:
                    return new byte[] { 0x81, Convert.ToByte(len) };
                ...
                default:
                    throw new ArgumentException("The length cannot be greater than 0x7FFFFFFF.");
            }
        }
