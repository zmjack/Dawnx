        ...
        public void Test2()
        {
            Aes.Create().Self(_ =>
            {
                ...
                Assert.Equal("0c66182ec710840065ebaa47c5e6ce90",
                    _.Encrypt<AesEmptyIVCombiner>("123".GetBytes(Encoding.UTF8)).HexEncode());
            });

            Aes.Create().Self(_ =>
            {
                var now = DateTime.Now.ToString();
                var encrypted = _.Encrypt(now.ToString().GetBytes(Encoding.UTF8)).Base64Encode();
                Assert.Equal(now, _.Decrypt(encrypted.Base64Decode()).GetString(Encoding.UTF8));
            });
        }
