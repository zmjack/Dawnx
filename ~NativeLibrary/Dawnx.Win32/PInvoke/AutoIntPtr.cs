...
namespace Dawnx.Win32.PInvoke
{
    public sealed class AutoIntPtr<T> : IDisposable
    {
        ...
        public uint ULength { get; private set; }
        ...
        public AutoIntPtr(uint length)
        {
            if (length > int.MaxValue) throw new ArgumentException("The `length` must be less than or equel to int.MaxValue(2147483647)");
            if (!IsSupportedType) throw new TypeLoadException($"The type {TypeFullName} is not supported.");

            if (TypeFullName == "System.Byte[]") ULength = length;
            else ULength = (uint)Marshal.SizeOf(typeof(T));

            Ptr = Marshal.AllocHGlobal((int)ULength);
        }
        ...
        public T Value
        {
            ...
            set
            {
                ...
                Marshal.Copy(buffer, 0, Ptr, (int)ULength);
            }
        }
