...
namespace Dawnx.Algorithms.ApplyingAlgorithm.Test
{
    public class DistributorTests
    {
        ...
        public void Test1()
        {
            ...
            var result = distributor.Distribute(new DistributionContract<DefaultProducer, DefaultConsumer>
            {
        ...
        public void MutilProducerTest()
        {
            ...
            var distributor = new Distributor(DistributionMethod.FormerPreferred);
            var result = distributor.Distribute(new DistributionContract<DefaultProducer, Bucket>
            {
                Producers = producers,
                ProducerAmount = x => x.Amount,
                Consumers = consumers,
