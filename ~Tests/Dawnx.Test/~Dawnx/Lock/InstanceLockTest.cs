...
namespace Dawnx.Test.Lock
{
    public class InstanceLockTest
    {
        ...
        public void Test()
        {
            var ModelLock_YearMonth = InstanceLock<Model>.Get(x => x.Year, x => x.Month, x => x.Sex, x => "const");
        ...
        {
            Assert.ThrowsAny<ArgumentException>(() => InstanceLock<Model>.Get(x => x.Year, x => x.Obj));
        }
